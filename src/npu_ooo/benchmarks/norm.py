...
def build_layernorm_model(*, rows: int = 128, cols: int = 96) -> ModelSpec:
    graph = OperatorGraph(
        graph_id="row_layernorm_graph",
        tensors=(
            TensorSpec("X", ("M", "N")),
            TensorSpec("Y", ("M", "N")),
        ),
        operators=(
            OperatorSpec(
                ...
                reduction_dims=(("N", "N"),),
                attributes={"axis": "N", "epsilon": 1e-5},
                provenance={"template_op": "layernorm"},
            ),
        ),
        attributes={"pattern": "row_layernorm"},
    )
    return ModelSpec(
        model_id="row_layernorm",
        family=ModelFamily.ENCODER_TRANSFORMER,
        variant="row-layernorm-v0",
        shape_symbols=(("M", rows), ("N", cols)),
        templates=(GraphTemplate("row_layernorm", graph, parameters=("M", "N")),),
        top_level_template="row_layernorm",
        dtype_policy="fp16",
    )


def build_layernorm_case(
    *,
    architecture_profile: str = "minimal",
    scheduler_profile: str = "sequential",
    evaluation_scope: EvaluationScope = EvaluationScope.ONE_BLOCK,
