...
def plan_uniform_tiles(graph: OperatorGraph, *, tile_size: int = 32) -> ScheduleSpec:
    ...
    for operator in graph.operators:
        ...
        # Every ordinary transform has an output-domain tile space.  The
        # lowering layer maps each output tile back to source regions; only a
        # stateful cache update needs one complete tensor schedule.
        is_broadcast = bool(
        ...
        schedules.append(
            OperatorSchedule(
                ...
                tile_sizes=tuple(
                    (
                        name,
                        int(extent) if full_tensor_transform else min(tile_size, int(extent)),
                    )
                ...
                attributes={
                    "full_tensor_transform": full_tensor_transform,
                },
            )
        )
    result = ScheduleSpec(
